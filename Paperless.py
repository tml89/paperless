...
def getDate(text, pdfFile):
    ...
    if len(text) > 0:
        for m in re.finditer(DATE_REGEX, text):
            date = dateparser.parse(m.group(0), settings={
    ...
    Logger.Log("Date in Text not found return creation date",
...
for sourceFilename in os.listdir(Settings.NEXTCLOUDROOT + Settings.SOURCEDIR):
    sourceFilePath = os.path.join(
        Settings.NEXTCLOUDROOT + Settings.SOURCEDIR, sourceFilename)
    ...
    if text is None or len(text) < 50:
        try:
            ...
            ocrmypdf.ocr(sourceFilePath, sourceFilePath, language='deu',
                         skip_text=True, deskew=True, rotate_pages=True, progress_bar=False)
            continue
    ...
    if currDestDir != Settings.DESTDIR:
        notify.SendNotification(
            "Paperless, verschiebe Datei: " + sourceFilename, "Nach: " + currDestDir, "tim")

    # build destination path
    destFilePath = Settings.NEXTCLOUDROOT + currDestDir + \
        "/[" + fileDate + "]" + sourceFilename
    Logger.Log("DestFilePath: " + destFilePath, Logger.LogLevel.Debug)
    Logger.Log("Move: " + sourceFilename + " to: " +
               destFilePath, Logger.LogLevel.Info)
    try:
